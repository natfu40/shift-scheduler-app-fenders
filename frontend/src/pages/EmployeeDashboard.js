...
function EmployeeDashboard() {
  ...
  const getCalendarSubscriptionUrl = () => {
    ...
    const baseUrl = `${API_CONFIG.BASE_URL}${API_ENDPOINTS.CALENDAR.USER_SHIFTS_ICS}`;
  ...
  const handleDownloadICS = () => {
    const token = localStorage.getItem('token');
    const url = `${API_CONFIG.BASE_URL}${API_ENDPOINTS.CALENDAR.USER_SHIFTS_ICS}?token=${encodeURIComponent(token)}`;
