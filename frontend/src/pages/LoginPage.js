...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { token, userId, firstName, lastName, firstTimeLogin: isFirstTimeLogin } = response.data;
      ...
      try {
        ...
        isAdmin = adminResponse.data === true;

        // Fallback for default admin if API check fails
        if (!isAdmin && isDefaultAdmin) {
          isAdmin = true;
        }
      } catch (err) {
        // Fallback for admin check failure
        if (isDefaultAdmin) {
          isAdmin = true;
        } else {
          isAdmin = false;
